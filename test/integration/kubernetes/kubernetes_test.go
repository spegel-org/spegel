  ...
  [plugins."io.containerd.metadata.v1.bolt"]
    content_sharing_policy = "isolated"`
			...
			createOpts := []cluster.CreateOption{
				cluster.CreateWithNodeImage(fmt.Sprintf("kindest/node:%s", tt.kubernetesVersion)),
			...
			require.NoError(t, err)
			slices.SortFunc(kindNodes, func(a, b kindnodes.Node) int {
				aRole, _ := a.Role()
				bRole, _ := b.Role()
				if aRole == "control-plane" {
					return -1
				}
				if bRole == "control-plane" {
					return 1
				}
				return 0
			})
...
[host.https://registry-1.docker.io]
  capabilities = [push]`
			...
			t.Log("Block internet access")
			for _, node := range kindNodes {
				err = node.CommandContext(t.Context(), "iptables", "-A", "OUTPUT", "-o", "eth0", "-d", "10.0.0.0/8", "-j", "ACCEPT").Run()
				require.NoError(t, err)
				err = node.CommandContext(t.Context(), "iptables", "-A", "OUTPUT", "-o", "eth0", "-d", "172.16.0.0/12", "-j", "ACCEPT").Run()
				require.NoError(t, err)
				err = node.CommandContext(t.Context(), "iptables", "-A", "OUTPUT", "-o", "eth0", "-d", "192.168.0.0/16", "-j", "ACCEPT").Run()
				require.NoError(t, err)
				err = node.CommandContext(t.Context(), "iptables", "-A", "OUTPUT", "-o", "eth0", "-j", "REJECT").Run()
				require.NoError(t, err)
				//TODO: Check that example.com cant be reached.
			}

			// Wait for content to advertise and system to settle.
		...
		failedPod := corev1.Pod{
			...
			Spec: corev1.PodSpec{
				...
				Containers: []corev1.Container{
					{
						Name:            "pull-test",
						Image:           "ghcr.io/spegel-org/benchmark:never-exists",
						ImagePullPolicy: corev1.PullAlways,
					},
				},
			},
...
func dumpPods(t *testing.T, k8sClient kubernetes.Interface, namespace string, includeLogs bool) {
	...
	for _, pod := range podList.Items {
		...
		output = append(output, fmt.Sprintf("%s (%s %d/%d %d) -> %s", pod.Name, pod.Status.Phase, readyCount, len(pod.Status.ContainerStatuses), restartCount, pod.Spec.NodeName))
		if includeLogs {
