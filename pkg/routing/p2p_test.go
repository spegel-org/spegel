...
func TestP2PRouter(t *testing.T) {
	...
	// Create main router with no peer to bootstrap with.
	mainBs := NewStaticBootstrapper(nil)
	mainRouter, err := NewP2PRouter(t.Context(), "localhost:0", mainBs, "9090", routerOpts...)
	require.NoError(t, err)
	g.Go(func() error {
		return mainRouter.Run(gCtx)
	})
	ready, err := mainRouter.Ready(t.Context())
	require.NoError(t, err)
	require.False(t, ready)
	mainIP, err := manet.ToIP(mainRouter.host.Addrs()[0])
	require.NoError(t, err)

	// Advertise and Withdraw nil keys should not error.
	err = mainRouter.Advertise(t.Context(), nil)
	require.NoError(t, err)
	err = mainRouter.Withdraw(t.Context(), nil)
	require.NoError(t, err)

	// Advertising while offline should not error.
	advertisedKey := "will find key"
	err = mainRouter.Advertise(t.Context(), []string{advertisedKey})
	require.NoError(t, err)

	// Lookup local key should not return self.
	bal, err := mainRouter.Lookup(t.Context(), advertisedKey, 3)
	require.NoError(t, err)
	_, err = bal.Next()
	require.ErrorIs(t, err, ErrNoNext)

	// Create routers that all bootstrap with the main router.
	routers := []*P2PRouter{}
	for range 30 {
		bs := NewStaticBootstrapper([]peer.AddrInfo{*host.InfoFromHost(mainRouter.host)})
	...
	require.EventuallyWithT(t, func(c *assert.CollectT) {
		...
		require.Equal(c, int64(1), mainRouter.prov.Stats().Operations.Past.KeysProvided, 1)
	...
	require.Equal(t, 30, mainRouter.kdht.RoutingTable().Size())
	...
	for _, r := range routers {
		...
		require.NoError(t, err)
		require.Equal(t, mainIP.String(), addrPort.Addr().String())
	...
	bal, err = mainRouter.Lookup(t.Context(), newKey, 3)
	require.NoError(t, err)
	addrPort, err := bal.Next()
	require.NoError(t, err)
