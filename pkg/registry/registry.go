...
func (r *Registry) handleMirror(rw mux.ResponseWriter, req *http.Request, ref reference) {
	...
	for {
		...
		case addrPort, ok := <-peerCh:
			...
			err := forwardRequest(r.client, r.bufferPool, req, rw, addrPort)
			if err != nil {
				log.Error(err, "request to mirror failed", "attempt", mirrorAttempts, "path", req.URL.Path, "mirror", addrPort)
				continue
			}
			log.V(4).Info("mirrored request", "path", req.URL.Path, "mirror", addrPort)
			return
		}
	}
}
