...
func TestRegistryHandler(t *testing.T) {
	...
	for i := range 2 {
		badReg, err := NewRegistry(storetest.NewMemory(nil, nil), routing.NewMemoryRouter(map[string][]routing.Peer{}, routing.Peer{}))
		require.NoError(t, err)
	...
	contents := []storetest.Content{
		...
		{MediaType: "dummy", Data: []byte("last peer working")},
	...
	memStore := storetest.NewMemory(contents, nil)
	...
	for i := range 3 {
		...
		flakyStore := &flakyStore{Memory: storetest.NewMemory(contents, nil)}
	...
	reg, err := NewRegistry(storetest.NewMemory(nil, nil), router, WithRegistryFilters([]oci.Filter{oci.RegexFilter{Regex: regexp.MustCompile(`:latest$`)}}))
	require.NoError(t, err)
	handler := reg.Handler(logr.Discard())
