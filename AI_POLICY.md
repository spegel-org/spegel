...
All PRs created with the help of an LLM should be clearly stated. You are expected to disclose the model used and to what extent it was used to generate the code. Any changes made without a human in the loop are explicitly forbidden. You are still expected to take responsibility for the changes that you submit which is not possible without human involvement.
