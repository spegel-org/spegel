...
func TestHostFileContent(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "example registry with resolve",
			resolveEnabled: true,
...
[host."http://127.0.0.1:5000".header]
  ...
  X-Spegel-Mirror = ["true"]`,
		...
		{
			name:           "example registry without resolve",
...
[host."http://127.0.0.1:5000".header]
  ...
  X-Spegel-Mirror = ["true"]`,
		...
		{
			name:           "multiple mirrors with resolve",
			resolveEnabled: true,
			registry:       "https://example.com",
...
[host."http://127.0.0.1:5001".header]
  ...
  X-Spegel-External = ["true"]`,
		...
		{
			name:           "multiple mirrors without resolve",
			resolveEnabled: false,
			registry:       "https://example.com",
			mirrors:        []string{"http://127.0.0.1:5000", "http://127.0.0.1:5001"},
			expectedContent: `server = "https://example.com"
  ...
  X-Spegel-Mirror = ["true"]`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			content := hostsFileContent(*registryURL, mirrorUrls, tt.resolveEnabled)
...
func TestMirrorConfiguration(t *testing.T) {
	...
	err := AddMirrorConfiguration(context.TODO(), fs, registryConfigPath, registries, mirrors, true)
	require.NoError(t, err)
